...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        img_labels = pd.read_csv(os.path.join(root, "image_class_labels.txt"), sep=" ", header=None,
                                 names=['idx', 'label'])
        train_test_split = pd.read_csv(os.path.join(root, "train_val_test_split.txt"), sep=" ", header=None,
                                       names=['idx', 'train_flag'])
        data = pd.concat([img_paths, img_labels, train_test_split], axis=1)
        data = data[data['train_flag'] == train_val_test]

        # split dataset
        data = data[data['label'] < num_select_classes]
        base_data = data[data['label'] < num_base_classes]
        novel_data = data[data['label'] >= num_base_classes]
